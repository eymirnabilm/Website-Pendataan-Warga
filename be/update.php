...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    if (is_numeric($id)) {
        ...
        if (isset($put_data['ktp'])) {
            $ktp = $put_data['ktp'];
        } else {
            $ktp = ''; // Atur ke nilai kosong jika tidak ada perubahan
        }
        ...
        if (isset($put_data['alamat'])) {
            $alamat = $put_data['alamat'];
        } else {
            $alamat = ''; // Atur ke nilai kosong jika tidak ada perubahan
        }

        $sql = "UPDATE ujian_data SET ktp='$ktp', nama='$nama', alamat='$alamat' WHERE id='$id'";
