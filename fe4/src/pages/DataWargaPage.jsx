...
const DataWargaPage = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get("http://localhost/EymirNabilMakarim_50421426_UJIAN/be/read.php");
  ...
  const deleteWrg = async (WrgId) => {
    try {
      await axios.delete(`http://localhost/EymirNabilMakarim_50421426_UJIAN/be/delete.php/${WrgId}`);
        ...
        <table className="w-full border">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="border border-black p-2">ktp</th>
            ...
            {WargaData.length > 0 ? (
              WargaData.map((item, index) => (
                <tr key={item.id} className="bg-white">
                  ...
                  <td className="border p-2">{item.ktp}</td>
                    ...
                    <div className="grid grid-cols-2 text-center gap-2 px-2">
                      ...
                      <button
                        onClick={() => {
                          if (
                            window.confirm(
                              "Apakah Anda yakin ingin menghapus Warga tersebut?"
                            )
                          ) {
                            deleteWrg(item.id);
                          }
